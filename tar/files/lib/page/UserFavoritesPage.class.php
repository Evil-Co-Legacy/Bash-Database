...
class UserFavoritesPage extends MultipleLinkPage {
	public $templateName = 'userFavorites';
	...
	public function readData() {
		...
		$sql = "SELECT
				...
				FROM
			   	...
			   	WHERE
			   		`bash".BASH_N."_favorite`.userID = ".$this->frame->getUserID().(!BASHCore::getUser()->getPermission('mod.bash.moderatorPermissions') ? ' AND bash'.BASH_N.'_entry.isDisabled = 0' : '')."
				ORDER BY
	...
	public function show() {
		...
		UserProfileMenu::getInstance()->setActiveMenuItem('wcf.user.profile.menu.link.bash.favorites');
