{include file='documentHeader'}
	<head>
		<title>{lang}bash.page.bash.title{/lang} - {PAGE_TITLE}</title>
		...
		<div id="main">
		    <div class="mainHeadline">
				...
				<div class="headlineContainer">
					<h2>{lang}bash.page.bash.title{/lang}</h2>
				</div>
		    </div>
			
			{if $userMessages|isset}{@$userMessages}{/if}
			{if $entry->isDisabled}<p class="info">{lang}bash.page.bash.disabledInfo{/lang}</p>{/if}
