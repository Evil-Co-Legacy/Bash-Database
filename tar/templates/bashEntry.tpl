				...
				<div class="messageFooter">
					<div class="smallButtons">
						<ul>
							...
							{if $this->user->userID > 0}<li><a id="favorite{@$entry->entryID}" href="index.php?action=Favorite{if !$isFavorite}Add{else}Remove{/if}&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}" title="{lang}bash.entry.{if $isFavorite}un{/if}favorite{/lang}"><img src="{icon}{if $isFavorite}delete{else}favorites{/if}S.png{/icon}" alt="" /> <span>{lang}bash.entry.{if $isFavorite}un{/if}favorite{/lang}</span></a></li>{/if}
