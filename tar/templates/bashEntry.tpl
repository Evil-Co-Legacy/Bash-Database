				...
				<div class="messageFooter">
					<div class="smallButtons">
						<ul>
							...
							{if $this->user->userID > 0}<li><a id="favorite{@$entry->entryID}" href="index.php?action=Favorite{if !$isFavorite}Add{else}Remove{/if}&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}" title="{lang}bash.entry.{if $isFavorite}un{/if}favorite{/lang}" onclick="BashEntryEditor.{if $isFavorite}unF{else}f{/if}avorite({@$entryID}); return false;"><img src="{icon}{if $isFavorite}delete{else}favorites{/if}S.png{/icon}" alt="" /> <span>{lang}bash.entry.{if $isFavorite}un{/if}favorite{/lang}</span></a></li>{/if}
							...
							{if $this->user->userID > 0 && $this->user->userID == $entry->authorID || $this->user->userID > 0 && $this->user->getPermission('mod.bash.moderatorPermissions')}<li><a id="delete{@$entry->entryID}" href="index.php?action=BashEntryDelete&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}" title="{lang}bash.entry.delete{/lang}" onclick="if(confirm('{lang}bash.entry.delete.confirm{/lang}')) BashEntryEditor.remove({@$entryID}); return false;"><img src="{icon}deleteS.png{/icon}" alt="" /> <span>{lang}bash.entry.delete{/lang}</span></a></li>{/if}
							{if $this->user->userID > 0 && !$entry->hasVoted() && $this->user->userID != $entry->authorID}<li><a id="vote{@$entry->entryID}Positive" onclick="BashEntryEditor.votePositive({$entry->entryID}); return false" href="index.php?action=BashEntryVotePositive&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}" title="{lang}bash.entry.vote.positive{/lang}" onclick="BashEntryEditor.votePositive({@$entryID}); return false;"><img src="{icon}addS.png{/icon}" alt="" /></a></li>{/if}
							{if $this->user->userID > 0 && !$entry->hasVoted() && $this->user->userID != $entry->authorID}<li><a id="vote{@$entry->entryID}Negative" onclick="BashEntryEditor.voteNegative({$entry->entryID}); return false" href="index.php?action=BashEntryVoteNegative&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}" title="{lang}bash.entry.vote.negative{/lang}" onclick="BashEntryEditor.voteNegative({@$entryID}); return false;"><img src="{icon}removeS.png{/icon}" alt="" /></a></li>{/if}
