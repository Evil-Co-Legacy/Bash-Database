			...
			<div class="border {if $this|method_exists:'getUserProfileMenu' && $this->getUserProfileMenu()->getMenuItems('')|count > 1}tabMenuContent{else}content{/if}">
				<div class="container-1 gallery">
					{if $entries|count > 0}
						<div class="userProfileBox">
							<div class="userProfileBoxInner">
								<h3 class="subHeadline">{lang}bash.user.entries.title{/lang} <span>({#$items})</span></h3>
					...
					{else}
						<div class="userProfileBox">
							<div class="userProfileBoxInner">
								<h3 class="subHeadline">{lang}bash.user.entries.title{/lang}</h3>
									{lang}bash.user.entries.noEntries{/lang}
							</div>
						</div>
					{/if}					
				</div>
