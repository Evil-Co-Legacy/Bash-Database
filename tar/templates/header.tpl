<div id="wrapper">
	...
	<div id="headerContainer">
		...
		<div id="userPanel" class="userPanel">
			<div class="userPanelInner">
				<p id="userNote">
					{if $this->user->userID != 0}{lang}bash.header.userNote.user{/lang}{else}{lang}bash.header.userNote.guest{/lang}{/if}
				</p>
				<div id="userMenu">
					<ul>
						{if $this->user->userID != 0}
							<li id="userMenuLogout"><a href="index.php?action=UserLogout&amp;t={@SECURITY_TOKEN}{@SID_ARG_2ND}"><img src="{icon}logoutS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.logout{/lang}</span></a></li>
							<li id="userMenuProfileEdit"><a href="index.php?form=UserProfileEdit{@SID_ARG_2ND}"><img src="{icon}editS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.profile{/lang}</span></a></li>
							<li id="userMenuFavorites"><a href="index.php?page=FavoriteList{@SID_ARG_2ND}"><img src="{icon}favoritesS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.favorites{/lang}</span></a></li>
	
							{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
	
							{if $this->user->isModerator()}
								<li id="userMenuModeration"{if $this->user->getOutstandingModerations()} class="new"{/if}><a href="index.php?page=ModerationOverview{@SID_ARG_2ND}"><img src="{icon}moderatorS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.moderation{/lang}{if $this->user->getOutstandingModerations()} ({#$this->user->getOutstandingModerations()}){/if}</span></a></li>
							{/if}
							{if $this->user->getPermission('admin.general.canUseAcp')}
								<li id="userMenuACP"><a href="acp/index.php?packageID={@PACKAGE_ID}"><img src="{icon}acpS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.acp{/lang}</span></a></li>
							{/if}
						{else}
							<li id="userMenuLogin" class="options"><a href="index.php?form=UserLogin{@SID_ARG_2ND}" id="loginButton"><img src="{icon}loginS.png{/icon}" alt="" id="loginButtonImage" /> <span>{lang}bash.header.userMenu.login{/lang}</span></a></li>
	
							{if !REGISTER_DISABLED}<li id="userMenuRegistration"><a href="index.php?page=Register{@SID_ARG_2ND}"><img src="{icon}registerS.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.register{/lang}</span></a></li>{/if}
	
							{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
	
							{if $this->language->countAvailableLanguages() > 1}
								<li id="userMenuLanguage" class="languagePicker options"><a id="changeLanguage" class="hidden"><img src="{icon}language{@$this->language->getLanguageCode()|ucfirst}S.png{/icon}" alt="" /> <span>{lang}bash.header.userMenu.changeLanguage{/lang}</span></a>
		...
		<div id="header">
			 ...
			 * $searchFieldTitle=title of search input field; default=language variable bash.header.search.query
			...
			{if !$searchFieldTitle|isset}{assign var='searchFieldTitle' value='{lang}bash.header.search.query{/lang}'}{/if}
			{if !$searchFieldOptions|isset}
				{capture assign=searchFieldOptions}
					<li><a href="index.php?form=Search&amp;action=24h{@SID_ARG_2ND}">{lang}bash.search.entriesOfTheLast24Hours{/lang}</a></li>
				{/capture}
			{/if}
			...
			<div id="search">
				<form method="post" action="{@$searchScript}">
					...
					<div class="searchContainer">
						...
						{@SID_INPUT_TAG}
						{if $searchHiddenFields|isset}{@$searchHiddenFields}{else}<input type="hidden" name="types[]" value="bashEntry" />{/if}
							...
							<div class="searchInputMenu">
								<div class="hidden" id="searchInputMenu">
									<div class="pageMenu smallFont">
										<ul>
											{@$searchFieldOptions}
											{if $searchShowExtendedLink}<li><a href="{@$searchExtendedLink}{if !$searchFieldValue|empty}&amp;defaultQuery={$searchFieldValue|rawurlencode}{/if}">{lang}bash.header.search.extended{/lang}</a></li>{/if}
										</ul>
									</div>
								</div>
							</div>
						...
						{if $searchShowExtendedLink}
							<noscript>
								<p><a href="{@$searchExtendedLink}">{lang}bash.header.search.extended{/lang}</a></p>
							</noscript>
						{/if}
					</div>
				</form>
			...
			<div id="logo">
				<div class="logoInner">
					...
					{elseif $this->getStyle()->getVariable('page.logo.image.application.use') == 1}
						<a href="index.php?page=Index{@SID_ARG_2ND}" class="pageLogo">
							<img src="{@RELATIVE_BASH_DIR}images/bash-header-logo.png" title="{lang}{PAGE_TITLE}{/lang}" alt="" />
						</a>
					{/if}
				</div>
			</div>
	...
	{capture append=userMessages}
		{if $this->user->userID}
			...
			{if $this->session->isNew}<p class="info">{lang}bash.header.welcomeBack{/lang}</p>{/if}
		...
		{elseif !$this->session->spiderID}
			<p class="info">{lang}bash.user.guestInfo{/lang}</p>
		
			{if $this->session->isNew}<p class="info">{lang}wcf.user.register.welcome{/lang}</p>{/if}
	
		{/if}
		{if OFFLINE == 1 && $this->user->getPermission('user.bash.canViewBashOffline')}
			<div class="warning">
				{lang}bash.global.offline{/lang}
